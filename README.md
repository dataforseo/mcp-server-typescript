...
The same tool implementations power both the CLI and MCP server. One binary serves both modes.
...
npx dataforseo-mcp-server docs index         # CLI
```

**HTTP MCP (preferred):** run Streamable HTTP and let the client use OAuth. Env credentials are optional fallback only.

**stdio MCP:** OAuth is not used on this transport — set env credentials (or pass them in the MCP client `env` block):
...
|--------|---------|--------|---------|
| Node (CLI + MCP) | `npm run build` (`tsc`) | `dist/index.js` | `bin`, Docker, `start*` scripts |
